```@meta
EditURL = "<unknown>/../examples/hubbard_chain.jl"
```

# Example 1: Hubbard Chain

In this first example we will work through an example for simulating the repulsive Hubbard model
...
Lastly, the Hamiltonian is at half-filling, and is particle-hole symmetric, when the chemical potential is zero, ``\mu = 0.0``.
...
function run_hubbard_chain_simulation(sID, U, μ, β, L, N_burnin, N_updates, N_bins; filepath = ".")
    ...
    # This type helps keep track where data will be written to, and also what verions of
    ...
    # For convenience, we seed this function a randomly sampled number for the global random number generator.
    seed = abs(rand(Int))
    rng = Xoshiro(seed)

    # Set the discretization in imaginary time in the DQMC simulation.
    Δτ = 0.10

    # Calculate the length of the imaginary time axis, Lτ = β/Δτ
    # The function simply dqmcf.eval_length_imaginary_axis() simply helps ensure
    ...
    # Initialize an additional information dictionary.
    # This of this as a notebook where you can store any parameters you would like to record
    ...
    # The struct describes the size of the finite periodic lattice that is simulated.
    ...
    # Define nearest-neighbor bond on 1D chain lattice.
    ...
    # Define nearest-neighbor hopping amplitude, setting the energy scale in the system.
    t = 1.0
    ...
    # It fully define the system being simulated, and also defines all the
    # various type of ID definitions (ORBITAL_ID, BOND_ID, HOPPING_ID, PHONON_ID)
    ...
    # can in general simulate a system where is random disorder in the on-site energer.
    ...
    # Measure the equal-time pair local s-wave pair correlation (`time-displaced = false`),
    ...
    # Initialize the sub-directory in the data folder that the various measurement
    # will be written to.
    ...
    # Initialize the spin-up and spin-down electron Green's function matrices, calculate
    # their respective determinants as the same time.
    ...
    # Record the final frequency for numerical stabilization that the system settled on.
    ...
    # Write simulation summary to file as TOML file.
    # This simulation summary filerecord the version number of SmoQyDQMC and Julia
    # used to perform the simulation. The dictionally `additional_info` is appended
   ...
   # and none of the binned data has been deleted from it.
    ...
    # aditionally writing final results to CSV files.
