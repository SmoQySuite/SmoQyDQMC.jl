...
function run_simulation()
   ...
   lattice = lu.Lattice(
    ...
    ossh_phonon = PhononMode(orbital = 1, Ω_mean = Ω)

    # add ossh phonon to electron-phonon model
    ossh_phonon_id = add_phonon_mode!(
        electron_phonon_model = electron_phonon_model,
        phonon_mode = ossh_phonon
    )
    ...
    ossh_coupling = SSHCoupling(
        model_geometry = model_geometry,
        tight_binding_model = tight_binding_model,
        phonon_modes = (ossh_phonon_id, ossh_phonon_id),
        bond = bond,
        α_mean = α
    )
    ...
    initialize_correlation_measurements!(
        ...
        time_displaced = true,
        pairs = [(ossh_phonon_id, ossh_phonon_id)]
    )
    ...
    for n in 1:N_burnin
        ...
        (accepted, logdetG, sgndetG) = swap_update!(
            ...
            fermion_greens_calculator = fermion_greens_calculator,
            fermion_greens_calculator_alt = fermion_greens_calculator_alt,
            B = B, rng = rng, phonon_type_pairs = ((ossh_phonon_id, ossh_phonon_id),)
        )
        ...
        (accepted, logdetG, sgndetG) = reflection_update!(
            ...
            fermion_greens_calculator = fermion_greens_calculator,
            fermion_greens_calculator_alt = fermion_greens_calculator_alt,
            B = B, rng = rng, phonon_types = (ossh_phonon_id,)
        )
