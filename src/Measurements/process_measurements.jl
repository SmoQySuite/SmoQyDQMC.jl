...
When the first passed arguments is `comm::MPI.Comm`, then the rebinning of data can be parallelized using MPI.
However, it will only be parallelized if `parallelize_rebinning = true`.
...
- `parallelize_rebinning::Bool = true`: If `comm::MPI.Comm` is passed as first argument to this function,
   then if this is true then rebinning is done on each MPI process and then collected together on the root process
   where jackknife is used to calculate mean and error of observables.
   Otherwise all the rebinning is performed by the root process alone.
- `n_bins::Union{Int, Nothing} = nothing`: Number of bins used per walker to calculate statistics.
   If `nothing` then set equal to the number of data bins written to file during the simulation.
   Must be a factor of the number of bins written to file per walker during the DQMC simulation.
- `pIDs::Union{Int,Vector{Int}} = Int[]`: Specifies for which process IDs to calculate average statistics.
   If `pIDs = Int[]`, the calculate for all process IDs.
   If `comm::MPI.Comm` is passed as first function argument then `pIDs` must be of type `Vector{Int}` and not `Int`.
- `filename_prefix::String = "stats"`: Start of filename for HDF5 containing final statistics.
   HDF5 files containing statistics for a single process ID will end with `pID-$(pID).h5`.
...
function process_measurements(
    ...
    # KEYWORD ARGUMENTS
    datafolder::String,
    parallelize_rebinning::Bool = true,
    ...
    if isone(length(pIDs))
        filename = @sprintf "%s_pID-%d.h5" filename_prefix pID
    else
    ...
    # if MPI parallelized rebinning done.
    if parallelize_rebinning
        # process measurements using parallelized rebinning
    ...
    elseif iszero(MPI.Comm_rank(comm))
        # then process the measurements on the root process.
...
function process_measurements(;
    # KEYWORD ARGUMENTS
    datafolder::String,
    parallelize_rebinning::Bool = true, # Ignored
    n_bins::Union{Int, Nothing} = nothing,
